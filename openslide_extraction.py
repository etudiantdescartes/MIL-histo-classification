...
def draw_grid(wsi, rect_list, extraction_level, grid_path):
    ...
    for rect in rect_list:
        draw.rectangle(rect, outline='green', width=10)
...
def is_tissue(tile, check_area=True, tissue_area=0.5):
    ...
    Checks if a specific area is covered by some tissue if check_area is True
    """
    rgb = np.array(tile)
    hsv = rgb2hsv(rgb)
...
def extract_tiles(wsi_path, tile_size, extraction_levels, destination, output_grid):
    ...
    #deepzoom_added_levels = 10 look into it, weird behavior, 9 levels added with deepzoom
    ...
    for extraction_level in extraction_levels:
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                if is_tissue(tile, check_area=True, tissue_area=0.5):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', dest='source', default='wsi')
    parser.add_argument('-d', dest='destination', default='patches')
    ...
    extraction_levels = sorted([2,4,3])
    output_grid = True
    ...
    #TODO: handle memory error for very large images
    #TODO: same in feature_extration.py (might be solved by handling the error in this script though)
